...
class angkor_Helper
{
	...
	function getEmailsList(){
		...
		JPluginHelper::importPlugin('angkor');		
		$emails = array();
		$results = $dispatcher->trigger('onEmailsList', array(&$emails));
		//echo '<pre>';		print_R($emails);		echo '</pre>';
		return $results[0];		
	}
	function loadAssets(){
		$document = JFactory::getDocument();
		$url = JURI::root().'administrator/components/com_angkor/assets/css/angkor.css';
		$document->addStyleSheet($url);
		
		$view = JRequest::getCmd('view','angkor');
		//if($view=='css'){
			$url = JURI::root().'administrator/components/com_angkor/assets/codemirror/codemirror.js';			
			$document->addScript($url);
			
			$url = JURI::root().'administrator/components/com_angkor/assets/codemirror/css.js';			
			$document->addScript($url);
			
			$url = JURI::root().'administrator/components/com_angkor/assets/codemirror/codemirror.css';			
			$document->addStyleSheet($url);
	//	}
		//else{
			...
			$url = JURI::root().'administrator/components/com_angkor/assets/js/angkor.js';
			$document->addScript($url);	
		//}
	}
	function getAvailableFieldParameters($code)
	{
		...
		if($code)
		{
			...
			$dispatcher	=& JDispatcher::getInstance();
			JPluginHelper::importPlugin('angkor');		
			$results = $dispatcher->trigger('onDynamicAvailableFields', array ($code,&$available_fields));
	...
	function getEmail($code,$lang){
		...
		if($email){			
			...
			$dispatcher	=& JDispatcher::getInstance();
			JPluginHelper::importPlugin('angkor');		
			$results = $dispatcher->trigger('onDefaultEmail', array (&$data));
		}
		return $data;
	}
	function parsingEmailCSS($email,$embed_image=false,$css_content=''){
		if(!class_exists('Emogrifier'))
			require_once(JPATH_SITE.DS.'administrator'.DS.'components'.DS.'com_angkor'.DS.'helper'.DS.'emogrifier'.DS.'emogrifier.php');
		
		if($css_content==''){
			$css = angkor_Helper::getCSS();		
			$css_content = $css->css;
		}
		$Emogrifier = new Emogrifier($email->Body,$css_content);		
