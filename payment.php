...
$itemPriceWoc = rtrim($itemPrice,",");
$itemPrice = array_sum(explode(",",$itemPriceWoc));

//var_dump($itemPrice);exit;
        ...
        foreach($_FILES['invFile']['name'] as $key=>$mage){
            ...
            for($j=0;$j<count($ItemIdsArr);$j++){
                ...
                for($i=0; $i < count($mulfiles[$j]['name']) ; $i++){
                    ...
                    $status=Controlbox::submitpayshopperassist($CustId,$itemPrice,$card_number,$txtccnumberStr, $card_exp_month,  $txtNameonCardStr, $card_exp_year,$specialinstructionStr,$txtTaxesStr,$txtShippChargesStr,$invidkStr,$qtyStr,$articleStr,$txtPaymentMethod,$pg,'','',$tid,$mulfilename,$mulimageByteStream);
