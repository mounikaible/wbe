...
// Initialize form validation on the registration form.
   ...
   $joomla(".wrchild").click(function(){
        if($joomla('input[name="shipment"]:checked').val()=="1"){
            ...
            $joomla('#tabs2 #j_table tbody tr').each( function () {
                if($joomla(this).attr('id')==rs){ 
                  htmsea+='<tr id="dfg">'+$joomla(this).html()+'</tr>';
            ...
            if($joomla(this).val()=='-'){
               ...
               $joomla.each($joomla('#tabs2 #j_table tbody tr#dfg'), function(){
                ...
                if(rmv==1){
                 ...
                 $joomla(this).closest('tr').after(htmsea); 
                $joomla(this).val('-');
            }     
        }else{
...
if($rg->ItemImage==""){
              ...
              if(end(explode('.', $rg->ItemImage))=="pdf"){
                  $mgtd='<td class="action_btns"><a href="'.str_replace("https:","http:",$rg->ItemImage).'" target="-blank" ><i class="fa fa-eye"></i></a></td>';
              }else{
                  $mgtd='<td class="action_btns"><a class="ship_img" data-toggle="modal" data-backdrop="static" data-keyboard="false" href="#" data-id="'.str_replace("https:","http:",$rg->ItemImage).'" data-target="#view_image" ><i class="fa fa-eye"></i></a></td>';
              }
          }
             ...
             echo '<tr id="'.$rg->BillFormNo.'" style="display:none">
              		<td class="action_btns">
                        <input type="checkbox" name="txtId" class="txtId" value="'.$rg->ItemName.':'.$rg->BillFormNo.':'.$rg->ItemQuantity.':'.$rg->TrackingId.':'.$rg->ItemId.':'.$rg->ItemPrice.':'.$rg->cost.':'.$idf.':'.$volres.':'.$rg->ServiceType.':'.$rg->source.':'.$rg->destination.':'.$rg->MeasureUnits.':'.$sim.'">
              		</td>
