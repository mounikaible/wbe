        ...
        $joomla.ajax({
			...
			beforeSend: function() {
              ...
              $joomla('#ord_ship #step2 .btn-primary').attr("disabled", true);
		       ...
		       $joomla("#insuranceDiv").html(cosship[5]);
		       if(cosship[0]>0){
		       if(resv=="standard")
    			   $joomla('#divShipCOstOne').html('<label>Shipping Method - Standard</label><br><label>SHIPPING COST : </label>'+cosship[4]+'<br><label>INSURANCE COST : </label>'+cosship[5]+'<br><label>ADDITIONAL SERVICES : </label>'+cosship[2]+'<br><label>DISCOUNT : </label>'+cosship[3]+' </label><br><label>TOTAL COST : </label>'+ cosship[0]);
    			   else
    			   $joomla('#divShipCOstTwo').html('<label>Shipping Method -  Express </label><br><label>SHIPPING COST : </label>'+cosship[4]+'<br><label>INSURANCE COST : </label>'+cosship[5]+'<br><label>ADDITIONAL SERVICES : </label>'+cosship[2]+'<br><label>DISCOUNT : </label>'+cosship[3]+' </label><br><label>TOTAL COST : </label>'+cosship[0]);
        	   }else{
                  $joomla('input[name=shipmentStr]').filter(':radio').prop('checked',false);		           
...
<div class="container">
  <div class="main_panel persnl_panel">
    ...
    <div class="panel-body">
     ...
     <div id="loading-hold" style="display:none"><img src='/components/com_userprofile/images/loader.gif'></div>
        <div class="row ishpments" style="display:none">
          <div class="col-md-12">
            <div class="table-responsive">
              <table class="table table-bordered theme_table" id="u_table" data-page-length='50'>
                <thead>
                  <tr>
...
if($rg->ItemImage==""){
             ...
             if($p==$rg->BillFormNo){
             echo '<tr id="'.$rg->BillFormNo.'" style="display:none">
              		<td class="action_btns">
                        <input type="checkbox" name="txtId" class="txtId" value="'.$rg->ItemName.':'.$rg->BillFormNo.':'.$rg->ItemQuantity.':'.$rg->TrackingId.':'.$rg->ItemId.':'.$rg->cost.':'.$rg->cost.':'.$idf.':'.$volres.':'.$rg->ServiceType.':'.$rg->source.':'.$rg->destination.':'.$rg->MeasureUnits.':'.$sim.'">
              		</td>
             ...
             echo '<tr>
              		<td class="action_btns">
                        <input type="button" class="wrchild" value="+">
                        <input type="checkbox" name="txtId" value="'.$rg->ItemName.':'.$rg->BillFormNo.':'.$rg->ItemQuantity.':'.$rg->TrackingId.':'.$rg->ItemId.':'.$rg->cost.':'.$rg->cost.':'.$idf.':'.$volres.':'.$rg->ServiceType.':'.$rg->source.':'.$rg->destination.':'.$rg->MeasureUnits.':'.$sim.'">
              			<input type="button" name="ship" class="ship" data-id="'.$rg->ItemName.':'.$rg->BillFormNo.':'.$rg->ItemQuantity.':'.$rg->TrackingId.':'.$rg->ItemId.':'.$rg->cost.':'.$rg->cost.':'.$idf.':'.$volres.':'.$rg->ServiceType.':'.$rg->source.':'.$rg->destination.':'.$rg->MeasureUnits.':'.$sim.'" data-target="#ord_ship" title="SHIP">
              		</td>
              		<td><input type="hidden" name="txtbiladdress" value="'.UserprofileHelpersUserprofile::getBindShipingAddress($user,$rg->BillFormNo).'">'.$rg->BillFormNo.'</td>
              		<td>'.$rg->ItemName.'</td>
              		<td>'.$rg->TrackingId.'</td>
    ...
    ?>
        ...
        <div class="row ishpments2">
          <div class="col-md-12">
            <div class="table-responsive">
              <table class="table table-bordered theme_table" id="j_table" data-page-length='50'>
                <thead>
                  <tr>
                    <th class="action_btns">Actions#</th>
                    <th>Werehouse Receipt#</th>
      ...
      <div class="modal-body">
        ...
        <div id="step3" style="display:none">
          <form name="userprofileFormFive" id="userprofileFormFive" method="post" action="" enctype="multipart/form-data">
            ...
            <input type="hidden" name="emailStr" value="<?php echo $UserViews->PrimaryEmail;?>"/>sb-qa1ib1508141@personal.example.comJorge.farias@cifexpressusa.com-->
            ...
            <div class="row">
              <div class="col-md-12">
                <div class="finish-shipping">
                  ...
                  <table width="100%" class="table table-bordered theme_table shipping-costtbl">
                    ...
                    <tr>
                      ...
                      <td><label>INSURANCE CHARGES</label></td>
                      <td class="txt-right"><div id="insuranceDiv"></div></td>
                    </tr>
                    <tr>
                      <td><label>MYBOXSVD EXRESS COSTS</label></td>
                      <td class="txt-right">0.00</td>
                    </tr>
                    <tr class="total_cst">
                ...
                <div id="loading-image5" style="display:none"><img src="<?php echo JURI::base(); ?>/components/com_userprofile/images/loader.gif"></div>
                  ...
                  <div class="payment-method" id="dvPaymentMethod">
